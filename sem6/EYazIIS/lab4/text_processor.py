...
try:
    ...
    print("Модель en_core_web_md загружена")
except OSError:
    import subprocess
    import sys
    print("Загрузка модели en_core_web_md...")
...
try:
    ...
    # Скачиваем английскую модель (один раз, при первом запуске)
    stanza.download('en')
    # Создаём пайплайн с constituency парсером
    stanza_nlp = stanza.Pipeline('en', processors='tokenize,pos,constituency')
    print("✓ Stanza constituency parser ready")
except ImportError:
...
# Добавление GlossBERT (с обработкой ошибок)
try:
    from spacy_glossbert import has_glossbert_wsd
    if "glossbert_wsd" not in nlp.pipe_names:
        nlp.add_pipe("glossbert_wsd", last=True, config={
            "model_name": "kanishka/GlossBERT"
        })
    print("GlossBERT WSD компонент добавлен")
except Exception as e:
    print(f"WSD не загружен (будет работать только синтаксис): {e}")
...
class TextProcessor:
    ...
    def get_constituency_tree(self, sentence: str) -> str:
        """Строит дерево составляющих через Stanza"""
        if not STANZA_AVAILABLE or stanza_nlp is None:
            return "Stanza не установлен. Выполните: pip install stanza"
        
        try:
            doc = stanza_nlp(sentence)
            # Получаем дерево составляющих из первого предложения
    ...
    def _format_tree(self, tree, level=0):
        """Рекурсивно форматирует дерево в строку"""
        indent = "  " * level
        if hasattr(tree, 'children') and tree.children:
            children_str = " ".join([self._format_tree(child, level+1) for child in tree.children])
        ...
        for sent_text in sentences:
            ...
            for token in doc:
                ...
                token_data = {
                    ...
                    'position': token.i,  # позиция в предложении
                    'head_position': token.head.i  # позиция родителя
                }

                if hasattr(token._, 'glossbert_synset') and token._.glossbert_synset:
                    synset = token._.glossbert_synset
                    token_data['word_sense'] = synset.definition()
