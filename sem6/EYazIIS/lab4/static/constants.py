    ...
    <body>
        <div class="container">
            ...
            <div class="help-grid">
                ...
                <!-- Средняя колонка - Работа с синтаксическим деревом -->
                <div class="help-column">
                ...
                <!-- Правая колонка - Семантический анализ (новая) -->
                <div class="help-column">
                    <h2 class="semantic">🧠 Семантический анализ</h2>
                    
                    <div class="note semantic">
                        <strong>Что это?</strong> Определение точного значения слова в контексте. В отличие от синтаксического анализа (грамматическая структура), семантический отвечает на вопрос <strong>"Что это значит?"</strong>
                    </div>
                    
                    <h3 class="semantic">🤖 Как это работает</h3>
                    <p>В системе используется технология <strong>GlossBERT</strong> + <strong>WordNet</strong>:</p>
                    <ol>
                        <li>Анализируется контекст вокруг слова</li>
                        <li>Сравнивается с определениями из базы WordNet</li>
                    ...
                    <h3 class="semantic">🔍 Фильтрация</h3>
                    <p><strong>Поиск по лемме</strong> - введите часть слова, чтобы найти все вхождения с похожей леммой. Фильтрация происходит в реальном времени по мере ввода.</p>
                    
                    <h3 class="semantic">✏️ Редактирование значений</h3>
                    <p><strong>Клик на ячейку со значением</strong> позволяет:</p>
                    <ul>
                        <li>Исправить автоматически определенное значение</li>
                        <li>Добавить значение, если модель не смогла его определить</li>
                        <li>Уточнить или дополнить существующее определение</li>
                    </ul>
                    <p>Изменения сохраняются в базу данных и отображаются в таблице.</p>
                    
                    <h3 class="semantic">📊 Данные в таблице</h3>
                    <ul>
                        <li><strong>Лемма</strong> - нормализованная форма слова</li>
                        <li><strong>Слово</strong> - исходная форма в тексте</li>
                        <li><strong>Семантическое значение</strong> - определение из WordNet (редактируемое)</li>
                        <li><strong>Контекст</strong> - предложение, в котором употреблено слово</li>
                    </ul>
                    
                    <h3 class="semantic">💾 Экспорт</h3>
                    <p>Доступны два формата экспорта:</p>
                    <ul>
                        <li><strong>TXT</strong> - человекочитаемый текстовый файл с пояснениями</li>
                        <li><strong>JSON</strong> - структурированный формат для машинной обработки</li>
                    </ul>
                    
                    <h3 class="semantic">⚠️ Ограничения</h3>
                    <ul>
                        <li>Анализируются только <strong>существительные, глаголы и прилагательные</strong></li>
                        <li>Требуется подключение к интернету при первом запуске</li>
                        <li>Модель может ошибаться на редких значениях или сложных омонимах</li>
                        <li>При ошибке можно вручную исправить значение через клик по ячейке</li>
                    </ul>
                    
                    <div class="note semantic">
                        <strong>💡 Совет:</strong> Если модель часто ошибается для определенных слов в вашей предметной области, отредактируйте значения вручную — они сохранятся в базу и будут использоваться при последующих экспортах.
                    </div>
                    
                    <h3 class="semantic">📈 Статистика</h3>
                    <p>Вверху страницы отображается общее количество слов с семантическими значениями. При фильтрации счетчик обновляется автоматически.</p>
                </div>
            </div>
        </div>
    </body>
