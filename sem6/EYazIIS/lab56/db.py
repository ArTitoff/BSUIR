...
class CorpusDatabase:
    ...
    def add_tokens(self, sentence_id, tokens_list):
        ...
        with self.cursor() as cur:
            ...
            for token in tokens_list:
                ...
                if token.get('word_sense'):  # ← исправлено: .get() вместо прямого доступа
                    cur.execute("""
    ...
    # Запрос для поиска контекста llm

    def find_sentences_by_keywords(self, keywords: list, limit: int = 5) -> list:
        """
        Ищет предложения, содержащие ключевые слова (леммы)
        """
        if not keywords:
            return []
        ...
        with self.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT 
                    ...
                    array_agg(DISTINCT t.lemma) as matched_lemmas
                FROM sentences s
                JOIN tokens t ON t.sentence_id = s.id
                JOIN documents d ON d.id = s.doc_id
                WHERE t.lemma IN ({placeholders})
                GROUP BY s.id, d.filename, d.title
    ...
    def create_session(self, session_id: str) -> bool:
        ...
        with self.cursor() as cur:
            cur.execute("""
                INSERT INTO dialogue_sessions (session_id)
                VALUES (%s)
    ...
    def save_message(self, session_id: str, role: str, content: str, sources: list = None) -> int:
        """Сохраняет сообщение"""
        with self.cursor() as cur:
            cur.execute("""
                INSERT INTO dialogue_messages (session_id, role, content, sources)
                VALUES (%s, %s, %s, %s)
                RETURNING id
            """, (session_id, role, content, sources))
            
            message_id = cur.fetchone()[0]  # СОХРАНЯЕМ ID ДО UPDATE
            
            # Обновляем время сессии
            cur.execute("""
                UPDATE dialogue_sessions SET updated_at = CURRENT_TIMESTAMP
                WHERE session_id = %s
            """, (session_id,))
            
            return message_id  # ← возвращаем сохранённый ID
        ...
        with self.cursor() as cur:
            cur.execute("""
                SELECT session_id, created_at, updated_at,
                    (SELECT COUNT(*) FROM dialogue_messages WHERE session_id = s.session_id) as msg_count
    ...
    def delete_session(self, session_id: str) -> bool:
        """Удаляет сессию и все сообщения"""
        with self.cursor() as cur:
            cur.execute("DELETE FROM dialogue_sessions WHERE session_id = %s", (session_id,))
            return cur.rowcount > 0
    ...
    def delete_message(self, message_id: int) -> bool:
        ...
        with self.cursor() as cur:
            cur.execute("DELETE FROM dialogue_messages WHERE id = %s", (message_id,))
            return cur.rowcount > 0
    ...
    def update_message(self, message_id: int, new_content: str) -> bool:
        ...
        with self.cursor() as cur:
            cur.execute("""
                UPDATE dialogue_messages SET content = %s WHERE id = %s
