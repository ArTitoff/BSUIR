...
sc_result sc_complex_event_subscription_add_events_list(sc_complex_event_subscription * complex_event_subscription, sc_list events_list)
{
  if (complex_event_subscription == null_ptr)
    return SC_RESULT_NO;

  // if (complex_event_subscription->events_list != null_ptr)       ????? type_error with !=
  // 	sc_list_clear(&complex_event_subscription->events_list);

  complex_event_subscription->events_list = events_list;

  return SC_RESULT_OK;
}

sc_result sc_complex_event_subscription_add_one_event(sc_complex_event_subscription * complex_event_subscription, 
                                                      sc_event_subscription const * event_subscription)
{
  if (complex_event_subscription == null_ptr)
    return SC_RESULT_NO;

  sc_list_push_back(&complex_event_subscription->events_list, &event_subscription);

  return SC_RESULT_OK;
}

sc_bool list_subscription_equal(void* node1, void* node2){    ///??? interestin equal function. does it look good?
  return node1 == node2;
}

sc_result sc_complex_event_subscription_delete_one_event(sc_complex_event_subscription * complex_event_subscription, 
                                                         sc_event_subscription const * event_subscription)
{
  if (complex_event_subscription == null_ptr)
    return SC_RESULT_NO;
  ...
  sc_list_remove_if(&complex_event_subscription->events_list, (void*)&event_subscription, list_subscription_equal);

  return SC_RESULT_OK;
}


/*! Adds the specified sc-complex_event_subscription to the registration manager's events table.
 * @param manager Pointer to the sc-event_subscription registration manager.
 * @param event_subscription Pointer to the sc-complex_event_subscription to be added.
 * @return Returns SC_RESULT_OK if the operation is successful, SC_RESULT_NO otherwise.
 */

sc_result _sc_complex_event_subscription_manager_add(
    sc_event_subscription_manager * manager,
    sc_complex_event_subscription * complex_event_subscription)
{
  sc_hash_table_list * element_events_list = null_ptr;
  ...
  // if there are no events for specified sc-element, then generate new events list
  element_events_list =
      (sc_hash_table_list *)sc_hash_table_get(manager->events_table, TABLE_KEY(complex_event_subscription->subscription_addr));
  element_events_list = sc_hash_table_list_append(element_events_list, (sc_pointer)complex_event_subscription);
  sc_hash_table_insert(
      manager->events_table, TABLE_KEY(complex_event_subscription->subscription_addr), (sc_pointer)element_events_list);
...
/*! Removes the specified sc-complex_event_subscription from the registration manager's events table.
 * @param manager Pointer to the sc-event_subscription registration manager.
 * @param event_subscription Pointer to the sc-complex_event_subscription to be removed.
 * @return Returns SC_RESULT_OK if the operation is successful, SC_RESULT_ERROR_INVALID_PARAMS otherwise.
 */
sc_result _sc_complex_event_subscription_manager_remove(
    sc_event_subscription_manager * manager,
    sc_complex_event_subscription * complex_event_subscription)
{
  sc_hash_table_list * element_events_list = null_ptr;
  ...
  // remove event_subscription from list of events for specified sc-element
  element_events_list = sc_hash_table_list_remove(element_events_list, (sc_const_pointer)complex_event_subscription);
  if (element_events_list == null_ptr)
    sc_hash_table_remove(manager->events_table, TABLE_KEY(complex_event_subscription->subscription_addr));
  else
    sc_hash_table_insert(
        manager->events_table, TABLE_KEY(complex_event_subscription->subscription_addr), (sc_pointer)element_events_list);
...
sc_complex_event_subscription * sc_complex_event_subscription_new(
    sc_memory_context const * ctx,
    sc_addr subscription_addr,
    sc_list events_list,
    sc_pointer data,
    ...
    sc_event_subscription_delete_function delete_callback)
{
  ...
  // if (!sc_storage_is_element(ctx, events_list)) //??????????   How to check list correct?
  //   return null_ptr;

  sc_complex_event_subscription * complex_event_subscription = sc_mem_new(sc_complex_event_subscription, 1);
  complex_event_subscription->subscription_addr = subscription_addr;
  complex_event_subscription->events_list = events_list;
  complex_event_subscription->callback = callback;
  complex_event_subscription->callback_with_user = null_ptr;
  complex_event_subscription->delete_callback = delete_callback;
  complex_event_subscription->data = data;
  complex_event_subscription->ref_count = 1;
  complex_event_subscription->sc_active_event_count = events_list.size;
  sc_monitor_init(&complex_event_subscription->monitor);

  // register generated event_subscription
  sc_event_subscription_manager * manager = sc_storage_get_event_subscription_manager();
  _sc_complex_event_subscription_manager_add(manager, complex_event_subscription);

  return complex_event_subscription;
}

sc_complex_event_subscription * sc_complex_event_subscription_with_user_new(
    sc_memory_context const * ctx,
    sc_addr subscription_addr,
    sc_list events_list,
    sc_pointer data,
    ...
    sc_event_subscription_delete_function delete_callback)
{
  ...
  complex_event_subscription->subscription_addr = subscription_addr;
  complex_event_subscription->events_list = events_list;
  complex_event_subscription->callback = null_ptr;
  complex_event_subscription->callback_with_user = callback;
  complex_event_subscription->delete_callback = delete_callback;
  complex_event_subscription->data = data;
  complex_event_subscription->ref_count = 1;
  complex_event_subscription->sc_active_event_count = events_list.size;
...
sc_result sc_complex_event_subscription_destroy(sc_complex_event_subscription * complex_event_subscription)
{
  if (complex_event_subscription == null_ptr)
    return SC_RESULT_NO;
  ...
  if (_sc_complex_event_subscription_manager_remove(subscription_manager, complex_event_subscription) != SC_RESULT_OK)
  {
    sc_monitor_release_write(&complex_event_subscription->monitor);
    return SC_RESULT_ERROR;
  }

  // if (complex_event_subscription->delete_callback != null_ptr)
  //   complex_event_subscription->delete_callback(complex_event_subscription);   need add sc_complex_event_subscription.h content and change functions

  complex_event_subscription->ref_count = SC_EVENT_REQUEST_DESTROY;
  complex_event_subscription->sc_active_event_count = SC_EVENT_REQUEST_DESTROY;
  complex_event_subscription->subscription_addr = SC_ADDR_EMPTY;
  
  if (!sc_list_destroy(&complex_event_subscription->events_list))
  	return SC_RESULT_ERROR;
  	
  complex_event_subscription->callback = null_ptr;
  complex_event_subscription->callback_with_user = null_ptr;
  complex_event_subscription->delete_callback = null_ptr;
  complex_event_subscription->data = null_ptr;

  sc_storage * storage = sc_storage_get();
  if (storage != null_ptr)
  {
    sc_monitor_acquire_write(&emission_manager->pool_monitor);
    sc_queue_push(&emission_manager->deletable_events_subscriptions, complex_event_subscription);
    sc_monitor_release_write(&emission_manager->pool_monitor);
  }
  sc_monitor_release_write(&complex_event_subscription->monitor);
...
sc_result sc_complex_event_notify_element_deleted(sc_addr element)
{
  sc_hash_table_list * element_events_list = null_ptr;
  sc_complex_event_subscription * complex_event_subscription = null_ptr;
  ...
  if (element_events_list != null_ptr)
  {
    while (element_events_list != null_ptr)
    {
      complex_event_subscription = (sc_complex_event_subscription *)element_events_list->data;
...
sc_bool sc_complex_event_subscription_is_deletable(sc_complex_event_subscription const * complex_event_subscription)
{
  return complex_event_subscription->ref_count == SC_EVENT_REQUEST_DESTROY;
}

sc_pointer sc_complex_event_subscription_get_data(sc_complex_event_subscription const * complex_event_subscription)
{
  return complex_event_subscription->data;
}

sc_list sc_complex_event_subscription_get_event_type(sc_complex_event_subscription const * complex_event_subscription)
{
  return complex_event_subscription->events_list;
}

sc_addr sc_complex_event_subscription_get_element(sc_complex_event_subscription const * complex_event_subscription)
{
  return complex_event_subscription->subscription_addr;
}
