...
bool is_operand(char symbol) {
    bool value = symbol == '(' || symbol == ')' || symbol == '/' || symbol == '\\' || symbol == '!';
    return value;
}
...
bool valid_alphabet(string str) {
    for (int i = 0; i < str.size(); i++) {
        if (is_operand(str[i]) || is_symbol(str[i]) || is_constant(str[i]))
            continue;
        else
            return false;
    }
...
bool correct_brackets_count(string str) { 
    ...
    if (sign_count != brackets_count && sign_count != close_brackets_count)
        return false;
    return true;
}
...
bool sub_string_valid(const string str) {
    int formula_count = 1;
    for (int i = 0; i < str.size(); i++) {
        if (str[i] == '\\' && str[i + 1] == '/')
            formula_count++;
    }
    if (formula_count > 1 && str[0] != '(' && str[str.size() - 1] != ')')
        return false;
    return true;
}

bool incorrect_parentheses_use(string str) {
...
bool good_disjuction(string str) {
    for (int i = 0; i < str.size(); i++) {
        ...
        if (str[i] == '\\' && i > 0 && i + 2 < str.size()) {
            if ((str[i + 1] == '/' && (str[i - 1] == ')' || is_symbol(str[i - 1]) || is_constant(str[i - 1]))
                && (str[i + 2] == '(' || is_symbol(str[i + 2]) || str[i + 3] == '!' || is_constant(str[i + 2]))) ||
                (str[i - 1] == '/' && (is_symbol(str[i + 1]) || str[i + 1] == '(' || is_constant(str[i + 1])))) {
                continue;
            }
            else {
                return false;
...
bool good_conjuction(string str) {
    for (int i = 0; i < str.size(); i++) {
        if (str[i] == '/' && (i == 0 || i + 2 >= str.size()))
            return false;
        if (str[i] == '/' && i > 0 && i + 2 < str.size()) {
            if ((str[i + 1] == '\\' && (is_symbol(str[i - 1]) || str[i - 1] == ')' || is_constant(str[i - 1]))
                && (is_symbol(str[i + 2]) || str[i + 2] == '(' || is_constant(str[i + 2]))) ||
                (str[i - 1] == '\\' && (is_symbol(str[i + 1]) || str[i + 1] == '(' || is_constant(str[i + 1])))) {
                continue;
            }
            else {
...
bool correct_negation(string str) {
    for (int i = 0; i < str.size(); i++) {
        if (str[i] == '!') {
            if ((i > 0 && str[i-1] == '(') && (i < str.size() - 1 && is_symbol(str[i+1])))
                return true;
            else if (i > 0 && str[i - 1] == '(' && i < str.size() - 1 && str[i + 1] == '(')
                return true;
            else
                return false;
        }
...
bool correct_symbol_negation(string str) {
    for (int i = 0; i < str.size(); i++) {
        if (str[i] == '!') {
            if ((i > 0 && str[i - 1] == '(') && (i + 5 < str.size() && str[i + 1] == '(' && str[i + 2] == '!' && is_symbol(str[i + 3])))
                return false;
        }
    }
    return true;
...
bool correct_one_symbol_writting(string str) {
    for (int i = 0; i < str.size(); i++) {
        if ((is_symbol(str[i]) || is_constant(str[i])) && i > 0 && str[i - 1] == ')') {
            return false;
        }
        if ((is_symbol(str[i]) || is_constant(str[i])) && i + 1 < str.size() &&
            ( str[i + 1] == '(' || str[i + 1] == '!')) {
            return false;
        }
    }
...
bool no_duplicate_or_one_subformula_brackets(string str) {
    for (int i = 0; i < str.size(); i++) {
        if (i > 0 && (is_symbol(str[i]) || is_constant(str[i])) && (is_symbol(str[i - 1]) || is_constant(str[i - 1])))
            return false;
        if (i > 0 && i + 1 < str.size() && (is_symbol(str[i]) || is_constant(str[i])) && str[i - 1] == '(' && str[i + 1] == ')')
            return false;
    }
    return true;
}
...
void is_knf() {
    ...
    if (!valid_alphabet(str)) {
        cout << "\nФормула НЕ является КНФ! Алфавит\n";
        return;
    }
    else if (!incorrect_parentheses_use(str))
    {
    ...
    {
        cout << "\nФормула НЕ является КНФ! Конъюнкция \n";
        return;
    }
        ...
        cout << "\nФормула НЕ является КНФ! Отрицание\n";
        return;
    }
    else if (!correct_symbol_negation(str)) {
        cout << "\nФормула НЕ является КНФ! Двойное отрицание\n";
        return;
    }
    else if (!no_duplicate_or_one_subformula_brackets(str)) {
        cout << "\nФормула НЕ является КНФ! Повторяшки или скобки с одной формулой\n";
        return;
    }
    else if (!correct_one_symbol_writting(str)) {
        cout << "\nФормула НЕ является КНФ! Ошибка при прописи одного символа\n";
        return;
    }
    else if (!correct_brackets_count_for_all_knf(str)) {
        cout << "\nФормула НЕ является КНФ! Скоюки для всей формулы\n";
        return;
    }
   else {
        ...
        for (int i = 0; i < splited_str.size(); i++) {
            //cout << splited_str[i];
            if (!correct_brackets_count(splited_str[i]) || !sub_string_valid(splited_str[i])) {
                cout << "\nФормула НЕ является КНФ! Подстрока\n";
                return;
            }
        } // ((A/\A)/\(B\/1))
    }
